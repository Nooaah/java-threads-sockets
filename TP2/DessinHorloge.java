...
public class DessinHorloge {
	...
	/*
	 * Dessin d'une aiguille Les paramétres sont l'angle (en fait la propotion
	 * d'angle de 0 à 1) 1 correspond à un angle de 2*PI la position x et y
	 * souhaitée pour le centre la longueur et la largeur de l'aiguille sa couleur
	 */
	public static void dessinerAiguille(Graphics gTmp, double angle, int x, int y, double longueur, double largeur,
			Color couleur) {
	...
	public static void dessineHorloge(Graphics gTmp, int haut, int larg, int heure, int minutes, double secondes) {
		...
		Color cFond = new Color(255, 255, 255);
		...
		DessinHorloge.dessinerNumeros(gTmp, 1, taille, Color.black);

		int centreX = taille / 2;
		int centreY = taille / 2;

		// Aiguille des secondes
		DessinHorloge.dessinerAiguille(gTmp, secondes / 60.0, centreX, centreY, 0.45 * taille, 0.01 * taille,
				Color.red);

		// Aiguille des minutes
		DessinHorloge.dessinerAiguille(gTmp, minutes / 60.0, centreX, centreY, 0.50 * taille, 0.02 * taille,
				Color.black);

		// Aiguille des heures
		DessinHorloge.dessinerAiguille(gTmp, heure / 12.0 + minutes / 60.0 / 12.0, centreX, centreY, 0.25 * taille,
				0.03 * taille, Color.gray);
